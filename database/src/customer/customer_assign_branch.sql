    ...
    BEGIN TRY
        BEGIN TRANSACTION;
            ...
            IF dbo.fn_customer_validate_id(@customer_id) = 0
                THROW 71000, 'Invalid customer_id. Customer does not exist.', 1;

            -- Validate employee id
            IF dbo.fn_employee_validate_id(@employee_id) = 0
                THROW 71001, 'Invalid employee_id. Employee does not exist.', 1;

            -- Validate new branch id
            IF dbo.fn_branch_validate_id(@new_branch_id) = 0
                THROW 71002, 'Invalid new_branch_id. Branch does not exist.', 1;

            -- Reject no-op transfer
            IF dbo.fn_customer_validate_branch(@customer_id, @new_branch_id) = 1
                THROW 71003, 'Customer already belongs to this branch.', 1;
            ...
            IF @@ROWCOUNT = 0
                THROW 71004, 'Failed to assign customer to branch.', 1;
