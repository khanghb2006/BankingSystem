    ...
    BEGIN TRY
        BEGIN TRANSACTION;
            ...
            IF dbo.fn_account_validate_id(@account_id) = 0
                THROW 72000, 'Invalid account_id. Account does not exist.', 1;
            
            -- Validate Account Role
            IF dbo.fn_account_validate_role(@account_id, 'Customer') = 0
                THROW 72001, 'Invalid account role. Account must be a Customer.', 1;

            -- Validate Account Status
            IF dbo.fn_account_validate_status(@account_id, 'Active') = 0
                THROW 72002, 'Invalid account status. Account must be Active.', 1;

            -- Validate Customer Profile Existence
            IF dbo.fn_customer_validate_exists(@account_id) = 1
                THROW 72003, 'Customer profile already exists for this account.', 1;

            -- Validate Branch ID
            IF dbo.fn_branch_validate_id(@branch_id) = 0
                THROW 72004, 'Invalid branch_id. Branch does not exist.', 1;

            -- Validate Customer's Citizen ID
            IF dbo.fn_customer_validate_citizen_id(@citizen_id) = 1
                THROW 72005, 'Invalid citizen_id. Citizen ID already exists.', 1;
            ...
            IF @@ROWCOUNT = 0
                THROW 72006, 'Failed to create customer profile.', 1;
        COMMIT TRANSACTION;
