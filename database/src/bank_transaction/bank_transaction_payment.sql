    ...
    BEGIN TRY
        BEGIN TRANSACTION;
            ...
            IF dbo.fn_bank_account_validate_id(@from_bank_account_id) = 0
                THROW 230000, 'Source bank account does not exist.', 1;

            -- Validate to_bank_account_id
            IF dbo.fn_bank_account_validate_id(@to_bank_account_id) = 0
                THROW 230010, 'Destination bank account does not exist.', 1;

            -- Validate accounts are different
            IF @from_bank_account_id = @to_bank_account_id
                THROW 230020, 'Source and destination accounts must be different.', 1;

            -- Validate amount
            IF @amount <= 0
                THROW 230030, 'Amount must be greater than 0.', 1;
            ...
            IF @@ROWCOUNT = 0
                THROW 230040, 'Insufficient balance or source account is not active.', 1;
            ...
            IF @@ROWCOUNT = 0
                THROW 230050, 'Destination account is not active.', 1;
