    ...
    BEGIN TRY
        BEGIN TRANSACTION;
            ...
            IF dbo.fn_account_validate_id(@account_id) = 0
                THROW 120000, 'Invalid account ID.', 1;
            
            -- Validate old password
            IF dbo.fn_account_validate_password(@account_id, @old_password) = 0
                THROW 120010, 'Old password is incorrect.', 1;

            -- new password should not be the same as the old password
            IF @old_password = @new_password
                THROW 120020, 'New password cannot be the same as the old password.', 1;
            ...
            IF @@ROWCOUNT = 0
                THROW 120030, 'Failed to update password.', 1;
